defmodule Rubber.Snapshot.RepositoryTest do
  @moduledoc """
  Tests for the Rubber.Snapshot.Repository module functions.
  ...
  use ExUnit.Case
  alias Rubber.Snapshot.Repository
  ...
  setup do
    on_exit(fn ->
      Repository.delete(@test_url, "rubber_test_repository_1")
      Repository.delete(@test_url, "rubber_test_repository_2")
    end)
  ...
  describe "constructing paths" do
    test "make_path should make url from repository name and query params" do
      assert Repository.make_path("rubber_test_unverified_backup", verify: false) ==
               "/_snapshot/rubber_test_unverified_backup?verify=false"
    end

    test "make_path should make url from repository_name" do
      assert Repository.make_path("rubber_test_repository_1") ==
               "/_snapshot/rubber_test_repository_1"
    end
  end
  ...
  describe "verifying a repository" do
    test "a registered but unverified repository is manually verified" do
      ...
      assert {:ok, %{status_code: 200, body: %{"nodes" => _}}} =
               Repository.verify(@test_url, "rubber_test_repository_1")
    end
  end
  ...
  describe "retrieving information about a repository" do
    ...
    test "information about all repositories" do
      Repository.register(@test_url, "rubber_test_repository_1", @test_repository_config)
      Repository.register(@test_url, "rubber_test_repository_2", @test_repository_config)
    ...
    test "information about a specific repository" do
      ...
      assert {:ok, %{status_code: 200}} =
               Repository.get(@test_url, "rubber_test_repository_1")
    end
  end
